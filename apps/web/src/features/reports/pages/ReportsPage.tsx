...
export function ReportsPage() {
  ...
  // Tabs
  const [activeTab, setActiveTab] = useState<'overview' | 'profit' | 'comparison'>('overview');
  ...
  // Previous Period Baseline Mock calculations for comparison
  const prevRevenue = totalRevenue * 0.85; // 15% lower baseline
  const prevCount = Math.round(totalOrders * 0.9);

  // Category sales breakdown
  const categoryReports: CategoryReport[] = categories
    .map((cat) => {
  ...
  const productReports: ProductReport[] = Object.entries(productSalesMap)
    .map(([id, info]) => ({
      id,
      name: info.name,
      sku: info.sku,
  ...
  return (
    <div className="space-y-6">
      ...
      {datePreset === 'custom' && (
        <Card className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 grid grid-cols-2 gap-4 animate-in slide-in-from-top-2 duration-200">
          <Input
            label="Start Date"
            type="date"
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6 animate-in fade-in duration-200">
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
            <Card className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm col-span-2 flex items-center justify-between">
              <div>
            ...
            <Card className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm col-span-2 flex items-center justify-between">
              ...
              <div className="p-2.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-lg shrink-0">
                <TrendingUp className="h-5 w-5" />
              </div>
            </Card>

            <Card className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm col-span-1">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm lg:col-span-2 space-y-4">
                ...
                <TableBody>
                  {productReports.map((p) => (
                    <TableRow key={p.id} className="hover:bg-transparent">
                      ...
                      <TableCell className="text-right text-xs text-slate-400">₦{p.cost.toLocaleString()}</TableCell>
